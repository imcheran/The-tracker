        ...
        <div className="shrink-0 mb-6 px-4">
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <button onClick={onMenuClick} className="md:hidden p-3 bg-surface-light dark:bg-surface-dark rounded-xl shadow-soft dark:shadow-soft-dark text-slate-500 hover:text-slate-800 dark:hover:text-white transition-all">
                        <Menu size={20} />
                    </button>
                ...
                {!isNotesView && (
                    <div className="flex items-center gap-3">
                        <button 
                            ...
                            className="p-3 bg-surface-light dark:bg-surface-dark rounded-xl shadow-soft dark:shadow-soft-dark text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-all hover:-translate-y-0.5 active:translate-y-0 active:shadow-soft-pressed dark:active:shadow-soft-dark-pressed"
                        >
                            {isSimplifiedView ? <EyeOff size={20} /> : <Eye size={20} />}
                        </button>
                        <button className="p-3 bg-surface-light dark:bg-surface-dark rounded-xl shadow-soft dark:shadow-soft-dark text-slate-500 hover:text-slate-700 transition-all hover:-translate-y-0.5 active:translate-y-0">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar pb-28 px-4 rounded-3xl">
            {filteredTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center opacity-50">
                    <div className="w-32 h-32 mb-6 flex items-center justify-center bg-surface-light dark:bg-surface-dark rounded-full shadow-soft-pressed dark:shadow-soft-dark-pressed">
                ...
                isNotesView ? (
                    ...
                    <div className="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
                        {filteredTasks.map(note => (
                             <div key={note.id} className="break-inside-avoid bg-surface-light dark:bg-surface-dark p-4 rounded-2xl shadow-soft dark:shadow-soft-dark mb-4 cursor-pointer hover:-translate-y-1 transition-transform" onClick={() => onSelectTask(note.id)}>
