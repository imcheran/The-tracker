...
const Sidebar: React.FC<SidebarProps> = React.memo(({ 
  ...
  const navItemClass = (view: ViewType | string) => `
    ...
    ${currentView === view 
        ...
        : 'text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:-translate-y-0.5 hover:shadow-soft-sm dark:hover:shadow-soft-dark-sm bg-surface-card_light dark:bg-surface-card_dark'
    }
  `;
  ...
  return (
    <>
        ...
        <div className="flex flex-col h-full py-4 pl-4 pr-2">
            <div className="flex-1 flex flex-col bg-surface-card_light dark:bg-surface-card_dark rounded-[2rem] shadow-soft dark:shadow-soft-dark overflow-hidden relative border border-white/20 dark:border-white/5">
        ...
        {isManagingLists && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/30 backdrop-blur-md p-4" onClick={() => setIsManagingLists(false)}>
                <div className="bg-surface-card_light dark:bg-surface-card_dark w-full max-w-sm rounded-[2rem] p-6 shadow-2xl animate-scale-in border border-white/20" onClick={e => e.stopPropagation()}>
